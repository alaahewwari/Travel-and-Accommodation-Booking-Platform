...
namespace TABP.Application.Users.Commands.Register
{
    public class RegisterUserCommandHandler(
        ...
        IPasswordHasher passwordHasher) : IRequestHandler<RegisterUserCommand, Result<UserResponse>>
    {
        public async Task<Result<UserResponse>> Handle(RegisterUserCommand request, CancellationToken cancellationToken)
        {
            ...
            if(role is null)
            {
                return Result<UserResponse>.Failure(RoleErrors.RoleNotFound);
            }
            var existingUser = await userRepository.GetUserByEmailAsync(request.Email,cancellationToken);
            if (existingUser is not null)
            {
                return Result<UserResponse>.Failure(UserErrors.UserAlreadyExist);
            }
            ...
            return Result<UserResponse>.Success(createdUser.ToUserResponse());
        }
